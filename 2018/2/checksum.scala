def checksum(boxIdList: List[String]): Int = {
  val twice = boxIdList.map(boxId => boxId.map(c => boxId.count(_ == c))).filter(boxIds => boxIds.contains(2)).size
  val thrice = boxIdList.map(boxId => boxId.map(c => boxId.count(_ == c))).filter(boxIds => boxIds.contains(3)).size
  twice * thrice
}

def commonChars(boxIdList: List[String]): String = {
  def matchesAllButOne(a: String, b: String): Boolean = {
    val aZ = a.zipWithIndex
    val bZ = b.zipWithIndex
    aZ.map(cA => cA._1 == bZ(cA._2)._1).filter(it => !it).size == 1
  }
  val theTwo = boxIdList.map(boxId => boxIdList.filter(anotherBoxId => matchesAllButOne(boxId, anotherBoxId))).filter(_.nonEmpty).flatten
  theTwo(0).intersect(theTwo(1))
}
